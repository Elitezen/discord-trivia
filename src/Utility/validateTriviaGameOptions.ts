...
function validatePlayerCount(label: 'minPlayerCount' | 'maxPlayerCount', val:unknown) {
  if (!val && val != 0) {
...
function checkPlayerCountRelation(min:number, max:number) {
  if (min > max) throw new DiscordTriviaError(
...
const validateTriviaGameOptions = (obj:TriviaGameOptions):void => {
  try {
    ...
    checkPlayerCountRelation(obj.minPlayerCount as number, obj.maxPlayerCount as number);
